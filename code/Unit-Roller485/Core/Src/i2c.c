...
uint8_t I2C1_TransmitData(uint8_t device_id, uint8_t *pdata, uint8_t size, uint16_t timeout)
{
    ...
    LL_I2C_HandleTransfer(I2C1, device_id, LL_I2C_ADDRSLAVE_7BIT, size, LL_I2C_MODE_AUTOEND,
...
uint8_t I2C1_TransmitData_RepeatedStart(uint8_t device_id, uint8_t *pdata, uint8_t size, uint16_t timeout)
{
    ...
    LL_I2C_HandleTransfer(I2C1, device_id, LL_I2C_ADDRSLAVE_7BIT, size, LL_I2C_MODE_SOFTEND,
                          LL_I2C_GENERATE_START_WRITE);
...
uint8_t I2C_Write_16bits_reg_Bytes(uint8_t DeviceAddress, uint16_t MemAddress, uint8_t *pData, uint8_t TxLen,
                                   uint16_t Timeout)
{
...
uint8_t I2C_Read_16bits_reg_Bytes(uint8_t DeviceAddress, uint16_t MemAddress, uint8_t *pData, uint8_t RxLen,
                                  uint16_t Timeout)
{
    uint8_t mem_address[2] = {(uint8_t)MemAddress, (uint8_t)(MemAddress >> 8)};

    if (!I2C1_TransmitData_RepeatedStart(DeviceAddress, mem_address, 2, Timeout)) {
        return I2C1_ReceiveData(DeviceAddress, pData, RxLen, Timeout);
