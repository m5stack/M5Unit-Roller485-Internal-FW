...
volatile uint8_t i2c_receive_overflow      = 0;
...
void i2c1_reset_receive_state(void)
{
    ubReceiveIndex       = 0;
    i2c_receive_overflow = 0;
}

void i2c1_start_timeout_monitor(void)
{
    i2c_stop_timeout_flag    = 0;
    i2c_stop_timeout_counter = 0;
    i2c_stop_timeout_delay   = HAL_GetTick();
    i2c_stop_timeout_flag    = 1;
}

void i2c1_reset_timeout_state(void)
{
    i2c_stop_timeout_flag    = 0;
    i2c_stop_timeout_counter = 0;
    i2c_stop_timeout_delay   = HAL_GetTick();
...
void Slave_Reception_Callback(void)
{
  ...
  RXNE flag is cleared by reading data in RXDR register */
    uint8_t received_data = LL_I2C_ReceiveData8(I2C1);

    if (ubReceiveIndex < I2C_RECEIVE_BUFFER_LEN) {
        aReceiveBuffer[ubReceiveIndex++] = received_data;
    } else {
