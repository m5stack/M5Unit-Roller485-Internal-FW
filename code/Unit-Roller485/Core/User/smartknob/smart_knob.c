...
{
    (void)consume_mechanical_delta_rad();
    current_detent_center  = 0;
...
{
    // If we are not moving and we're close to the center (but not exactly there), slowly adjust the centerpoint to
    // match the current position
    idle_check_velocity_ewma =
        motor_rps * IDLE_VELOCITY_EWMA_ALPHA + idle_check_velocity_ewma * (1 - IDLE_VELOCITY_EWMA_ALPHA);
    ...
    float delta_rad = consume_mechanical_delta_rad();
#if SK_INVERT_ROTATION
    delta_rad = -delta_rad;
#endif
    angle_to_detent_center += delta_rad;

    idle_delay = HAL_GetTick() - last_idle_start;
    rad_diff   = fabsf(angle_to_detent_center);
    if (last_idle_start > 0 && idle_delay > IDLE_CORRECTION_DELAY_MILLIS && rad_diff < IDLE_CORRECTION_MAX_ANGLE_RAD) {
        angle_to_detent_center *= (1 - IDLE_CORRECTION_RATE_ALPHA);
    }

    // Check where we are relative to the current nearest detent; update our position if we've moved far enough to snap
    // to another detent
    ...
    dead_zone_adjustment = CONSTRAIN(angle_to_detent_center,
    ...
    out_of_bounds = num_positions > 0 && ((angle_to_detent_center > 0 && current_position == config.min_position) ||
                                          (angle_to_detent_center < 0 && current_position == config.max_position));
    ...
    // Apply motor torque based on our angle to the nearest detent (detent strength, etc is handled by the PID_velocity
    // parameters)
