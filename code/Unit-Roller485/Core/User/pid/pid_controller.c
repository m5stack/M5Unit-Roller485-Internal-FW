...
// Functions
//*********************************************************************************
void PIDInit(PIDControl *pid, float kp, float ki, float kd, float sampleTimeSeconds, float minOutput, float maxOutput,
...
bool PIDCompute(PIDControl *pid)
{
    ...
    // Stop integrating further into saturation, while still allowing unwind.
    output_unsat = (pid->alteredKp) * error + (pid->iTerm) - (pid->alteredKd) * dInput;
    if (!((output_unsat >= pid->outMax && error > 0.0f) || (output_unsat <= pid->outMin && error < 0.0f))) {
...
void PIDOutputLimitsSet(PIDControl *pid, float min, float max)
{
    ...
    if (pid->mode == AUTOMATIC) {
        ...
        pid->output_prev = CONSTRAIN(pid->output_prev, min, max);
