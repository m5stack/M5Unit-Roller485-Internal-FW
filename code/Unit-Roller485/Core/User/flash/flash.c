...
bool writeMessageToFlash(uint8_t* buff, uint16_t length)
{
    ...
    /*Protection*/
    if (buff == NULL || length == 0 || (length % sizeof(uint64_t)) != 0 ||
        (length + sizeof(uint64_t)) > STM32G0xx_PAGE_SIZE) {
        return false;
    }
    ...
    if (STM32G0xx_FLASH_PAGE59_STARTADDR < (FLASH_BASE + FLASH_BANK_SIZE)) {
        My_Flash.Banks = FLASH_BANK_1;
        My_Flash.Page  = (STM32G0xx_FLASH_PAGE59_STARTADDR - FLASH_BASE) / STM32G0xx_PAGE_SIZE;
    } else {
        My_Flash.Banks = FLASH_BANK_2;
        My_Flash.Page  = (STM32G0xx_FLASH_PAGE59_STARTADDR - (FLASH_BASE + FLASH_BANK_SIZE)) / STM32G0xx_PAGE_SIZE;
    }
#else
    My_Flash.Banks = FLASH_BANK_1;
    My_Flash.Page  = (STM32G0xx_FLASH_PAGE59_STARTADDR - FLASH_BASE) / STM32G0xx_PAGE_SIZE;
    ...
    for (i = 0; i < length / 8; i++) {
        temp = buff[8 * i] | (uint64_t)buff[8 * i + 1] << 8 | (uint64_t)buff[8 * i + 2] << 16 |
               (uint64_t)buff[8 * i + 3] << 24 | (uint64_t)buff[8 * i + 4] << 32 | (uint64_t)buff[8 * i + 5] << 40 |
               (uint64_t)buff[8 * i + 6] << 48 | (uint64_t)buff[8 * i + 7] << 56;
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, STM32G0xx_FLASH_PAGE59_STARTADDR + 8 + 8 * i, temp) !=
            HAL_OK) {
            HAL_FLASH_Lock();
            return false;
        }
